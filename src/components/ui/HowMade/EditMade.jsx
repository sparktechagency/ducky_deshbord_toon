...
import { useAddMadeMutation, useGetMadeByIdQuery, useUpdateMadeMutation } from "../../../redux/apiSlices/madeSlice";
...
const EditMade = () => {
  ...
  const [addMade] = useAddMadeMutation();
  // Fetch existing made data
  const { data: made, isLoading: productLoading, refetch } = useGetMadeByIdQuery(id); // Assuming you have this query in Redux slice
  ...
  useEffect(() => {
    ...
    if (made) {
      form.setFieldsValue({
        ...
        details: made?.data?.details,
        category: made?.data?.category,
        firstName: made?.data?.firstName,
        secondName: made?.data?.secondName,
      });

      made?.data?.image1 && setImgURL1(refactorFileUrl(made?.data?.image1)); // Set the cover image URL
      made?.data?.image2 && setImgURL2(refactorFileUrl(made?.data?.image2)); // Set the cover image URL
      made?.data?.video && setVideoURL(refactorFileUrl(made?.data?.video)); // Set the cover image URL

      setCategory(made?.data?.category);
    }
  }, [made, form]);
  ...
  return (
    <Card title="Update How to Made" style={{ width: 1000, margin: "0 auto", }} className="bg-gray-200 shadow-md rounded-lg p-4">
      <Form
        form={form}
        layout="vertical"
        onFinish={onFinish}
